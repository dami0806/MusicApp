...
//Result 타입 사용시 성공, 실패 두가지로 나와서 에러타입을 정의해야함
...
final class NetworkManager {
    ...
    //let musicURL = "https://itunes.apple.com/search?media=music" --> constants안에 네임스페이스로 만듦
    // 네트워킹 요청하는 함수 (음악데이터 가져오기)
    typealias NetworkCompletion = (Result<[Music], NetworkError>) -> Void //타입 치환 <data, error>
    func fetchMusic(searchTerm: String, completion:@escaping NetworkCompletion){ //@escaping (Data?, Error?) -> Void)
